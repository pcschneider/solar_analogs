...
def fluxes(fn):
    ...
    for t in temps:
       ...
       for band in bands:
        ...
        band_expr = str(band[0])+" "+str(band[1])
        #band_expr = str("0.1 2.0")
        band_expr = str("0.1 2.4")
        print(band_expr)
        xs.AllModels.calcFlux(band_expr)
        flx = xs.AllData(1).flux
        print(flx)
        print(flx[0], ra[3])
        flux_per_count = float(flx[0])/float(ra[3])
        res[t].append([band, str("%5.2e" % flux_per_count)])
...
temps = [1.0, 1.2, 1.3, 1.4, 1.5, 2.0, 3.0]    
